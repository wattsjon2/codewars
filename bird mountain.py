    ...
    while False in checkList:
        for i in range(len(mountain)):
            ...
            if i == 0 or i == len(mountain) - 1:
                ...
                birdLevel = mountain[i].replace('^',f"{level + 1}")
                mountain.pop(i)
                ...
                for x in range(stringLen):               
                    if curString[x] == '^':
                        if curString[x-1] == " " and x != 0:
                            curString = curString[:x] + f"{level + 1}" + curString[x + 1:]
                        ...
                        elif curString[x-1] == str(level) and x != 0: 
                            curString = curString[:x] + f"{level + 1}" + curString[x + 1:]    
                        
                        elif x != stringLen - 1 and curString[x+1] == str(level):
                            curString = curString[:x] + f"{level + 1}" + curString[x + 1:] 

                        elif mountain[i-1][x] == str(level) or mountain[i+1][x] == str(level):
                            curString = curString[:x] + f"{level + 1}" + curString[x + 1:]
